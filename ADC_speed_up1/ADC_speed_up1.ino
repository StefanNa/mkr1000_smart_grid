...
const int cDiv = 30; //divide factor for generic clock
const float period = 100; //period of 100k sample rate
//String wFile = "ADC_DATA"; //used as file name to store wind and GPS data
volatile float aDCVal[dSize]; //array to hold ADC samples
...
void loop() {
  ...
  if(count==(dSize-1) and !done){ 
      float sTime = 0;
      for (int y = 0; y < dSize; y++) {

       Serial.println(aDCVal[y]); //write each reading to SD card as string
      }
...
void aDCSetup() {
  ...
  ADC->CTRLB.reg |= ADC_CTRLB_RESSEL_10BIT | ADC_CTRLB_PRESCALER_DIV32 | ADC_CTRLB_FREERUN; //This is where you set the divide factor, note that the divide call has no effect until you change Arduino wire.c
  //Wait for synchronization
  while (REG_ADC_STATUS & ADC_STATUS_SYNCBUSY);
...
void ADC_Handler() {
    if(count<1023) {
      ...
      //aDCVal[count]=0.9695*aDCVal[count-1]+0.0305*REG_ADC_RESULT;
      count++;
    }
    //curVal = REG_ADC_RESULT;
